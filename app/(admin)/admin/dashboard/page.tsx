...
import { FaBell, FaPlus, FaUsers } from "react-icons/fa";
...
const AdminDashboard = () => {
  ...
  const totalProfits = users.reduce(
    (total, item) => total + (item?.profitBalance || 0),
    0
  );

  const withdrawals = transactions.filter(
    (item) => item.category === "withdrawal" && item.status === "paid"
  );
  ...
  return (
    ...
    flex justify-center`}
        ...
        <div className="w-full flex items-center gap-4 justify-center flex-wrap">
          ...
          <DashboardCard
            title="Total Member Profits"
            detail={`${currency}${formatNumber(totalProfits)}`}
            icon={FaPlus}
          />
