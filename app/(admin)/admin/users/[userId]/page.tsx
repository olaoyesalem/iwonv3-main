...
import AddUserDepositModal from "@/components/admin/user/AddUserDepositModal";
...
import UserRefferals from "@/components/authenticated/account/UserRefferals";
...
import WithdrawProfitBalanceModalByAdmin from "./WithdrawProfitBalanceModalByAdmin";
import AddUserProfitBalanceModalByAdmin from "./AddUserProfitBalanceModalByAdmin";
...
const Page = () => {
  ...
  const [modals, setModals] = useState({
    ...
    userStatus: false,
    addUserProfit: false,
    addUserDeposit: false,
    withdrawUserProfit: false,
  });
  ...
  return (
    <>
        ...
        >
          ...
          <div className="flex flex-col w-full">
            ...
            <div className="w-full flex justify-between items-center py-4 border-b">
              ...
              <div>
                {currency}
                {formatNumber(user.profitBalance)}
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row w-full gap-3">
            <Button
              onClick={() => setModals({ ...modals, withdrawUserProfit: true })}
              label={"Withdraw"}
            />
            <Button
              onClick={() => setModals({ ...modals, addUserDeposit: true })}
              outline
              label={"Add Deposit"}
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row w-full gap-3">
            <Button
              onClick={() => setModals({ ...modals, addUserProfit: true })}
              label={"Add Profit Balance"}
            />
          </div>
        </div>
      </div>
      ...
      <div className="py-8">
        <UserRefferals title="User Referrals" username={user?.username} />
      </div>
      ...
      <AddUserDepositModal
        user={user}
        opened={modals.addUserDeposit}
        onClose={() => setModals({ ...modals, addUserDeposit: false })}
      />

      <AddUserProfitBalanceModalByAdmin
        user={user}
        opened={modals.addUserProfit}
        onClose={() => setModals({ ...modals, addUserProfit: false })}
      />

      <WithdrawProfitBalanceModalByAdmin
        user={user}
        opened={modals.withdrawUserProfit}
        onClose={() => setModals({ ...modals, withdrawUserProfit: false })}
      />
    </>
  );
};
