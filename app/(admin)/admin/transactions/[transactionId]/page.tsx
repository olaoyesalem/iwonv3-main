...
const Page = () => {
  ...
  const [transaction, setTransation] = useState<TransactionType>();
  const [beingPaid, setBeingPaid] = useState(false);
  ...
  async function paidHandler() {
    setBeingPaid(true);
    try {
      const { data } = await axios.put(`/api/transactions/${transactionId}`, {
        status: "paid",
      });
      setTransation(data);
    } catch (error) {
      ...
      <div className={`flex justify-center pb-7`}>
        ...
        >
          ...
          <div className="w-full flex items-center gap-3">
            ...
            >
              <Button label={"Transaction Owner"} />
            </Link>
          ...
          <div className="flex flex-col w-full">
            {transaction && (
              <div className="w-full flex justify-between items-center py-4 border-b capitalize">
                <div>{transaction?.category} Amount:</div>
                <div>
                  {currency}
            ...
            {transaction?.withdrawalAddress && (
              <div className="w-full flex justify-between items-center py-4 border-b">
                ...
                <CopyIconButton value={transaction?.withdrawalAddress}>
                  {transaction?.withdrawalAddress}
                </CopyIconButton>
              </div>
            )}
