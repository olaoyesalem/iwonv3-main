...
export const POST = async (request: Request) => {
  try {
    ...
    if (company.transfer.allowTransferIfPendingAvailable === "no") {
      const pendingTransaction = await Transaction.find<TransactionType>({
    ...
    const newSenderTransaction = new Transaction<Omit<TransactionType, "_id">>({
      title: "Transfer",
      amount: body.amount,
    ...
    const senderTransaction: TransactionType =
    ...
    if (company.transfer.mode === "direct-mode") {
      ...
      const receiverTransaction: TransactionType =
