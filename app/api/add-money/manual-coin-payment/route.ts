...
export const POST = async (request: Request) => {
  try {
    ...
    const actionNeededTransactions = await Transaction.find<TransactionType>({
      userId: user._id,
    ...
    const newTransaction = new Transaction<Omit<TransactionType, "_id">>({
      title: "Deposit",
      amount: body.amount,
      status: "action-needed",
      userId,
    ...
    const savedTransaction: TransactionType = await newTransaction.save();
