...
const ApproveDepositModal = (props: ApproveDepositProps) => {
  ...
  const handler = async () => {
    try {
      ...
      const { data } = await axios.patch(
        `/api/admin/transactions/deposit/approve/${transaction._id}`
      );
      if (data.error) throw new Error(data.error);
      toast.success(`${transaction.title} Aproved`);
      onClose();
      location.reload();
    } catch (error: any) {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3">
        <div>You want to Approve this transaction?</div>
