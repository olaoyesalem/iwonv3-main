...
const CompanyInfoModal = (props: ModalProps) => {
  ...
  const [data, setData] = useState({
    profitPercentage: company?.profitPercentage || 0,
    investPercentageFees: company?.investPercentageFees || 0,
    bannerProgressPercentage: company?.bannerProgressPercentage || 0,
    name: company?.name,
  ...
  const Handler = async () => {
    ...
    try {
      const reqData = {
        ...
        investPercentageFees: data.investPercentageFees,
        bannerProgressPercentage: data.bannerProgressPercentage,
      };

      setLoading(true);
      const res = await axios.post("/api/company/company-info", reqData);
      ...
      <div className="flex flex-col gap-3">
        <div className="w-full flex gap-3 flex-col">
          ...
          <div className="w-full flex gap-x-2 gap-y-2 flex-col sm:flex-row">
            ...
            <div className="w-full gap-1 flex-col hidden">
          ...
          <div className="w-full flex flex-col">
            <div className="font-semibold"> Profit Percentage </div>
            <NumberInput
              value={data.profitPercentage}
              onChange={(e) =>
                setData({ ...data, profitPercentage: Number(e.target.value) })
              }
            />
          </div>

          <div className="w-full flex flex-col">
            <div className="font-semibold"> Fees Percentage </div>
            <NumberInput
              value={data.investPercentageFees}
              onChange={(e) =>
                setData({
                  ...data,
                  investPercentageFees: Number(e.target.value),
                })
              }
            />
          </div>
          ...
          <div className="w-full flex flex-col">
            <div className="font-semibold"> Progressbar Percentage </div>
            <NumberInput
              value={data.bannerProgressPercentage}
              onChange={(e) =>
                setData({
                  ...data,
                  bannerProgressPercentage: Number(e.target.value),
                })
              }
            />
          </div>
