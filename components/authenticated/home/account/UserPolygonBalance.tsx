...
export default function UserPolygonBalance(props: Props) {
  const { mode } = useTheme();

  const { refreshing, refreshBalance, usdt, matic, set_refreshing } = props;
        ...
        ${mode === "light" ? "text-gray-800" : "text-white"}`}
          >
            {refreshing ? (
              ...
              <>
                <img
                  className="w-7 h-7 rounded-full bg-[#8247E5]"
                  src="/assets/img/icon/polygon.svg"
                  alt=""
                />
                <span className="tracking-wider">{matic.toFixed(3)}</span>
                <span className="ml-0.5">MATIC</span>
              </>
            )}
          </h3>

          {matic < 0.02 && !refreshing ? (
            <p className="text-sm bg-yellow-50 text-yellow-600 p-1.5 md:p-2 rounded-sm !font-light">
              ...
              to cover blockchain gas fees for withdrawals and deposits.
            </p>
          ) : null}
        </div>
      </div>
