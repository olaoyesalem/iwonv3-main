...
const AutomaticCoinPayment = (props: AutomaticCoinPaymentProps) => {
  ...
  const nowPaymentData = {
    ...
    ipn_callback_url: company?.baseUrl || "https://iwon.vc",
    order_id: userId,
  ...
  const addMoneyHandler = async () => {
    try {
      ...
      console.log("nowPaymentData ", nowPaymentData);
      console.log("company?.nowPaymentApi ", company?.nowPaymentApi);
      ...
      console.log("nowPaymentResult ", nowPaymentResult);
      ...
      const { _id: transactionId }: TransactionType = data;
